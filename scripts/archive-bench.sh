...
S_TAR_FILE=target/todo-linux-bench-standalone-x64.tar.gz
...
echo "========== tar.gz"
rm -f $TAR_FILE
tar -cvzf $TAR_FILE target/protostuffdb-bench target/protostuffdb-rmaster-bench target/hprotostuffdb-bench target/hprotostuffdb-rmaster-bench  $COMMON_FILES

echo "========== standalone tar.gz"
rm -f $S_TAR_FILE
tar -cvzf $S_TAR_FILE target/jre/*/ target/protostuffdb-rjre-bench target/protostuffdb-rmaster-rjre-bench target/hprotostuffdb-rjre-bench target/hprotostuffdb-rmaster-rjre-bench  $COMMON_FILES
